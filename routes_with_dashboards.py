...
# Dashboard Routes
@jobs_bp.route('/dashboard/service-provider')
@login_required
...
def service_provider_dashboard():
    ...
    with db.session() as session:
        ...
        recent_bookings = safe_run(session, """
            MATCH (u:User {id: $user_id})-[:PROVIDES]->(s:Service)<-[:BOOKED]-(b:Booking)
            MATCH (b)<-[:MADE_BOOKING]-(client:User)
        ...
        # Get recent reviews
        recent_reviews = safe_run(session, """
            MATCH (u:User {id: $user_id})-[:PROVIDES]->(s:Service)<-[:REVIEWS]-(r:Review)
            MATCH (r)<-[:WROTE]-(client:User)
            RETURN r, s.title as service_title, client.username as client_name
...
@jobs_bp.route('/dashboard/job-seeker')
@login_required
...
def job_seeker_dashboard():
    ...
    with db.session() as session:
        ...
        job_list = []
        for record in recommended_jobs:
            job_data = _node_to_dict(record['j'])
...
@jobs_bp.route('/businesses/dashboard')
@login_required
@role_required('business_owner')
...
def upload_verification():
    """Upload verification documents with role-specific requirements"""
    ...
    if form.validate_on_submit():
        ...
        with db.session() as session:
            ...
            files_uploaded = []
            upload_base_dir = os.path.join(current_app.config['UPLOAD_FOLDER'], 'verification')
            
            # Always require ID document
            if form.id_document.data:
                file = form.id_document.data
                filename = secure_filename(file.filename)
                role_dir = os.path.join(upload_base_dir, current_user.role, str(current_user.id))
                os.makedirs(role_dir, exist_ok=True)
                file_path = os.path.join(role_dir, f"id_{filename}")
                file.save(file_path)
                files_uploaded.append({'type': 'id', 'path': file_path})
            
            # Role-specific documents
            if current_user.role == 'job_seeker':
                # Job seeker: Resume required
                if form.resume.data:
                    file = form.resume.data
                    filename = secure_filename(file.filename)
                    role_dir = os.path.join(upload_base_dir, current_user.role, str(current_user.id))
                    os.makedirs(role_dir, exist_ok=True)
                    file_path = os.path.join(role_dir, f"resume_{filename}")
                    file.save(file_path)
                    files_uploaded.append({'type': 'resume', 'path': file_path})
                
                # Optional certifications
                if form.certifications.data:
                    for cert_file in form.certifications.data:
                        filename = secure_filename(cert_file.filename)
                        file_path = os.path.join(role_dir, f"cert_{filename}")
                        cert_file.save(file_path)
                        files_uploaded.append({'type': 'certification', 'path': file_path})
                
                verification_data['resume_file'] = files_uploaded[1]['path'] if len(files_uploaded) > 1 else None
                
            elif current_user.role == 'business_owner':
                # Business owner: Business permit required
                if form.business_permit.data:
                    file = form.business_permit.data
                    filename = secure_filename(file.filename)
                    role_dir = os.path.join(upload_base_dir, current_user.role, str(current_user.id))
                    os.makedirs(role_dir, exist_ok=True)
                    file_path = os.path.join(role_dir, f"permit_{filename}")
                    file.save(file_path)
                    files_uploaded.append({'type': 'business_permit', 'path': file_path})
                
                # Optional DTI registration
                if form.dti_registration.data:
                    file = form.dti_registration.data
                    filename = secure_filename(file.filename)
                    file_path = os.path.join(role_dir, f"dti_{filename}")
                    file.save(file_path)
                    files_uploaded.append({'type': 'dti_registration', 'path': file_path})
                
                verification_data['business_address'] = form.business_address.data
                verification_data['permit_number'] = form.permit_number.data
                verification_data['business_permit_file'] = files_uploaded[1]['path'] if len(files_uploaded) > 1 else None
                
            elif current_user.role == 'service_client':
                # Service provider: Sample work photos (optional)
                if form.sample_work.data:
                    for i, sample_file in enumerate(form.sample_work.data):
                        filename = secure_filename(sample_file.filename)
                        role_dir = os.path.join(upload_base_dir, current_user.role, str(current_user.id))
                        os.makedirs(role_dir, exist_ok=True)
                        file_path = os.path.join(role_dir, f"sample_work_{i+1}_{filename}")
                        sample_file.save(file_path)
                        files_uploaded.append({'type': 'sample_work', 'path': file_path})
        ...
        return jsonify({'message': 'Documents submitted successfully', 'redirect_url': url_for('dashboard.index')})
    
    return render_template('verification_upload_updated.html', form=form)
